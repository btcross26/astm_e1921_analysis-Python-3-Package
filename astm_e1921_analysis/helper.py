def ksi_to_MPa(stress_in_ksi):
    ...
    Arguments:
        stress_in_ksi: measurement in ksi
...
def in_to_mm(length_in_inches):
    ...
    Arguments:
        length_in_inches: measurement in inches
...
def lbf_to_newtons(force_in_lbf):
    ...
    Arguments:
        force_in_lbf: measurement in lbf
...
def stress_intensity_in_SI_to_english(K_in_MPa_root_meter):
    '''
    Convert a stress_intensity measurement in MPa root meters to ksi root inches
    
    Arguments:
        K_in_MPa_root_meter: measurement in MPa root meters
...
def adjust_KJc(KJc, B0, BX):
    '''
    Adjust a stress_intensity measurement in MPa root meters to account for size
    
    Arguments:
        KJc: KJc in MPa root meters        
        B0: actual specimen size        
        BX: size for which stress intensity should be calculated
        
    Returns:
        Adjust KJc in MPa root meters
    '''
...
def celsius_to_fahrenheit(temp_in_celsius):
    ...
    Arguments:
        temp_in_celsius: measurement in degrees celsius
...
def fahrenheit_to_celsius(temp_in_fahrenheit):
    ...
    Arguments:
        temp_in_fahrenheit: measurement in degrees fahrenheit
